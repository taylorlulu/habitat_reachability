...
class RearrangeTask(EmbodiedTask):
    ...
    def render(self, mode):
        ...
        print("当前机器人当前朝向为%.2f，expected_theta=%.2f"%(np.rad2deg(self._sim.robot.base_ori), np.rad2deg(expected_theta)))

        """第二步：获取物品Object并绘制"""
        obj_list = []
        test_handles = ['004_sugar_box_:0001', '004_sugar_box_:0003', '005_tomato_soup_can_:0000',
                        '007_tuna_fish_can_:0000', '008_pudding_box_:0001', '009_gelatin_box_:0003',
                        '009_gelatin_box_:0001', '024_bowl_:0002']
        ...
        """第三步：获取当前对应的states"""
        states, debug_points = states_utils.get_normal_state(self._sim.robot, tgt_obj, obj_list)

        # 调试部分：测试在机器人坐标系下的点是否正确
        robot_tf = mn.Matrix4(self._sim.robot.base_T)

        debug_line_render.push_transform(robot_tf)
        len_ = len(debug_points)
        for i in range(len_):
            points = debug_points[i]
            if points.ndim == 2:
                ...
                debug_line_render.draw_transformed_line(
                    mn.Vector3.zero_init(),
                    mn.Vector3(points[1, 0], points[1, 1], points[1, 2]),
                ...
                debug_line_render.draw_transformed_line(
                    mn.Vector3.zero_init(),
                    mn.Vector3(points[0], points[1], points[2]),
                    mn.Color4(1.0, 0.0, 0.0, 1.0),
                    mn.Color4(50.0, 1.0, 10.0, 1.0),
                )
        debug_line_render.pop_transform()
