...
class RearrangeTask(EmbodiedTask):
    ...
    def reset(self, episode: RearrangeEpisode):
        ...
        # 在此处获取target_receptacles和goal_receptacles物品
    ...
    def render(self, mode):
        ...
        # """调试：旋转后对应的观测差别很大"""
        # cur_robot_theta = self.cnt * 10 * np.pi / 180  # 当前期望的机器人角度
        # self._sim.robot.set_robot_pos(position=np.array([0.53050506, 0.12123175, 1.49464]), orientation=cur_robot_theta)
        # print("机器人当前的角度为%.2f"%(np.rad2deg(self._sim.robot.base_ori)))
        # self.cnt = (self.cnt + 1) % 36
        # print("机器人当前位置为%s"%(self._sim.robot.base_pos))
        # """调试：旋转后对应的观测差别很大"""
        ...
        # test_handles = ['024_bowl_:0002', '005_tomato_soup_can_:0000', '009_gelatin_box_:0001']
        ...
        # (states, self.curr_round_loc, round_actions) = states_utils.get_round_states(self._sim.robot, tgt_obj, obj_list)
        #
        # self.q = get_q_values(states, round_actions)
        #
        # # 测试由于角度不同导致的旋转
        # np.savetxt("debug_states/%d_q_values.txt" % (np.rad2deg(states_utils.limit_angle2(self._sim.robot.base_ori))), self.q, fmt='%.2f')
        #
        # """第四步：绘制可达性区域"""
        ...
        # 只需要在第一次进行求解
        if self.cnt == 0:
            (states, self.surround_loc, surrounding_actions) = states_utils.get_object_states(self._sim.robot, tgt_obj, obj_list)
            self.q = get_q_values(states, surrounding_actions)
            self.cnt += 1

        self._sim.show_surrounding_reachablity(self.q, self.surround_loc)
