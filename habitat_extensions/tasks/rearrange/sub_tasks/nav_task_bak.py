...
class RearrangeNavTaskV1(RearrangeNavTask):
    ...
    def render(self, mode):
        ...
        if self.cnt == 0:
            ...
            self.obj_list, self.points_global = states_utils.get_nearby_objects(self.container_obj, self.cur_tgt_T, self._sim.rigid_objs)
        ...
        debug_line_render.draw_box(
            bbox.min, bbox.max, mn.Color4(0.0, 0.0, 1.0, 1.0)
        )
        debug_line_render.pop_transform()

        # 将nav_goals转到机器人的局部坐标系下进行测试
        local_xyz = states_utils.transformation2robot_frame2(self.nav_goals, self._sim.robot.base_T)

        print("self.cnt=%s, local_xyz.shape=%s"%(self.cnt, self.nav_goals.shape))

        """第五步：获取桌子附近的states"""
        # 只需要在第一次进行求解
        if self.cnt == 0:
            (states, self.surround_loc, surrounding_actions) = states_utils.get_object_states2(self._sim.robot, self.cur_tgt_T,
                                                                                              self.obj_list, self.nav_goals)
            self.q = get_q_values(states, surrounding_actions)
            self.cnt += 1

        # 调试在局部坐标系下的坐标是否正确
        self._sim.show_surrounding_reachablity(self.q, local_xyz)
